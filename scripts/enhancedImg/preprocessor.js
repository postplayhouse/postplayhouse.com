		...
		async markup({ content, filename }) {
			...
			async function update_element(node, src_attribute) {
				...
				if (src_attribute.type === "MustacheTag") {
					...
					const src_var_name = content
						.substring(src_attribute.start, src_attribute.end)
						.trim()
					s.update(
						node.start,
						node.end,
				...
				if (OPTIMIZABLE.test(url)) {
					...
					const resolved_id = (await opts.plugin_context.resolve(url, filename))
						?.id
					if (!resolved_id) {
						const file_path = url.substring(0, url.indexOf("?"))
						if (
							existsSync(path.resolve(opts.vite_config.publicDir, file_path))
						) {
							throw new Error(
					...
					s.update(
						node.start + 1,
						node.start + 1 + "enhanced:img".length,
						"img",
					)
...
async function process(resolved_id, opts) {
	if (!opts.imagetools_plugin.load) {
		throw new Error(
			"Invalid instance of vite-imagetools. Could not find load method.",
		)
	}
	...
	return parseObject(
		code.replace("export default", "").replace(/;$/, "").trim(),
	)
}
...
function img_attributes_to_markdown(content, attributes, details) {
	...
	if (!user_width && !user_height) {
		...
		attribute_strings.push(
			`width=${Math.round(
				(stringToNumber(details.width) * user_height) /
					stringToNumber(details.height),
			)}`,
		)
	} else if (!user_height && user_width) {
		attribute_strings.push(
			`height=${Math.round(
				(stringToNumber(details.height) * user_width) /
					stringToNumber(details.width),
			)}`,
		)
	}
...
function img_to_picture(content, node, image) {
	...
	if (index >= 0) {
		sizes_string =
			" " + content.substring(attributes[index].start, attributes[index].end)
