...
<script module lang="ts">
	...
	const importMeta: Parameters<typeof makeFindImages>[0] = import.meta.glob(
		...
		`/src/images/seasons/2027/*.{avif,gif,heif,jpeg,jpg,png,tiff,webp,svg}` satisfies `/src/images/seasons/${(typeof site)["season"]}/*.{avif,gif,heif,jpeg,jpg,png,tiff,webp,svg}`,
		{
			eager: true,
			query: {
				enhanced: true,
