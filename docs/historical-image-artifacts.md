...
`generatorRevision` is an explicit publisher-code compatibility boundary. Bump
it whenever publisher logic could change generated bytes or `Picture` metadata;
that intentionally invalidates all transform keys.
